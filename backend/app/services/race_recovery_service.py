"""
Race Recovery service - Reconstruct a complete GPX track from a partial recording.

This module holds ALL the business logic for the race-recovery feature. The API
router (``app.api.race_recovery``) is only responsible for reading the uploaded
files and mapping errors raised here to HTTP responses.

Algorithm summary:
- Locate where the partial (watch) recording stops on the official track.
- Distribute the remaining (official) time over the missing segments using a
  slope-adjusted speed model, bounded to 30-200% of a base speed.
- Solve for the base speed with a 50-iteration binary search so the total
  reconstructed time matches the official finish time.
"""
...
from app.utils.elevation_quality import (
    assess_elevation_quality,
...
class RaceRecoveryError(Exception):
    """Domain validation error for the race-recovery flow.

    Raised for invalid inputs that the caller should surface as an HTTP 400
    (e.g. empty tracks, missing timestamps, official time shorter than the
    recorded time). The router maps this to ``HTTPException(400, str(err))``.
    """
...
def recover_race_track(
    incomplete_content: bytes,
    complete_content: bytes,
    official_time: str,
    approx_distance_km: Optional[str] = None,
) -> str:
    """Reconstruct a complete GPX track and return its XML representation.

    Combines a partial GPX recording (with timestamps, from a watch), a complete
    official track (usually without timestamps), and the official finish time to
    reconstruct the missing portion. Missing timestamps are computed from a
    slope-adjusted speed model whose base speed is solved via binary search so
    the total time matches the official finish time.

    Args:
        incomplete_content: Raw bytes of the partial GPX (must contain timestamps).
        complete_content: Raw bytes of the complete official GPX track.
        official_time: Official total time, formatted as ``HH:MM:SS`` or ``MM:SS``.
        approx_distance_km: Optional approximate distance covered (km) as a string,
            used to refine the cutoff-point search. Invalid values are ignored.

    Returns:
        The reconstructed GPX document serialized as an XML string.

    Raises:
        RaceRecoveryError: For invalid inputs (empty tracks, missing timestamps,
            or an official time not greater than the recorded time).
        ValueError: If ``official_time`` has an invalid format.
        gpxpy.gpx.GPXException: If either GPX payload cannot be parsed.
    """
    # Parse official time
    ...
    if not incomplete_points or not complete_points:
        raise RaceRecoveryError("GPX files must contain valid tracks")
    ...
    if first_time is None or last_time is None:
        raise RaceRecoveryError("Le GPX incomplet doit contenir des timestamps")
    ...
    if remaining_time_seconds <= 0:
        raise RaceRecoveryError(
            "Le temps officiel doit être supérieur au temps enregistré"
        )
    ...
    # Generate GPX XML
    return reconstructed_gpx.to_xml()
