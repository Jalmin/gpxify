          ...
          {gpxFiles.map((file, index) => {
            ...
            return (
              <div
                ...
                onDragEnd={onDragEnd}
                className={`flex items-center justify-between p-3 sm:p-4 rounded-lg border-2 ${colorScheme.border} ${colorScheme.bg} transition-all hover:scale-[1.02] cursor-move ${
              ...
              >
                <div className="flex items-center gap-2 sm:gap-4 min-w-0 flex-1">
                  ...
                  <div
                    className={`w-3 h-3 rounded-full flex-shrink-0 ${colorScheme.text}`}
                    style={{ backgroundColor: colorScheme.hex }}
                  ></div>
                  <div className="min-w-0 flex-1">
                    <div className={`font-semibold ${colorScheme.text} truncate text-sm sm:text-base`}>{file.filename}</div>
                    <div className="text-xs sm:text-sm text-muted-foreground flex items-center gap-2 sm:gap-3 mt-1">
                      <span className="whitespace-nowrap">{file.tracks.length} trace(s)</span>
                ...
                <button
                  ...
                  className="p-2 hover:bg-destructive/20 rounded-lg transition-colors group flex-shrink-0"
                >
                  <X className="w-4 h-4 sm:w-5 sm:h-5 text-muted-foreground group-hover:text-destructive" />
                </button>
              </div>
            );
          })}
